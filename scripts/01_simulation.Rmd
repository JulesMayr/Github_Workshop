...
While this can be seen as one big simulation, to make it easier to think through, it is split up into several different parts (stability metrics are always calculated):
...
* Part 3: This part manipulates beta diversity and area simultaneously
  ...
  *** e.g if the intended beta diversity is 2, then 2 any of those areas contain a combination of plots with 2 different species compositions
...
* Part 4: Aggregate subplots with different species compositions to obtain the maximum beta diversity for each area 
  ** i.e. beta diversity = 2 and Area = 2 etc. until beta diversity = 8 and Area = 8
  *** again, it needs to be noted that some subplots may share some species
  ** this is the final step of the simulation and is represented by the upper diagonal in Figure 3
  
**NOTE**: The entire script cannot be run in one single time as I includes what I call "Intermezzi" where I restart the R session and clear the global environment to not overload
          R's memory. The advantage of this, however, is that each part can also be run on its own once the previous parts have been run and key outputs have been saved.
...
#load data
load("../01_data/fake_data_11sp.RData")


#load VariancePartitioning Function for the calculation of stability metrics
source("../03_functions/Wang_et_al_2019_VariancePartitioning.r")
...
subplot_IDs <- unique(fake_data_11sp$unique_ID)

# for every subplot_ID:
for (i in 1:length(subplot_IDs)){ 
  datax = fake_data_11sp[fake_data_11sp$unique_ID==subplot_IDs[i],] # selects a subset from fake_data_11sp --> this subset contains rows only for sample i
...
Areas_unique_11sp[[1]] <-  subset(stability_data_scale1_11sp, select = -17) # add data for community stability to list without last column to maintain dimensions
      ...
      for(i in 1:resamplings){ 
        ...
        if (A==2) {
         ...
         scale_area.temp<-aggregate(total.biomass~year+div+unique_ID+sown_species_comp+an+ar+lu+or+po+tr+ve+fe+ru+ho+kn, data=temp, FUN=sum)
          ...
          # create a temporary data set that has only the two subplots we sampled 
          temp<-data.list.comp[[1]][[j]][data.list.comp[[1]][[j]]$unique_ID==year1scale_area[1]|data.list.comp[[1]][[j]]$unique_ID==year1scale_area[2]
...
variability_data_scale_area <- data.frame(sampleID=numeric(), CompID=character(), div=numeric(), Species_var=numeric(), Alpha_var=numeric(),  Metapopulation_var=numeric(),  Gamma_var=numeric(), Pop_synch=numeric(), Spatial_synch=numeric(), Species_synch=numeric(), Metapopulation_synch=numeric(), avg_richness=numeric(), Beta_div=numeric(), temp_mean=numeric(), temp_sd=numeric())
  ...
  for (i in 1:length(unique(scale_area$sampleID))){ 
    ...
    if(nrow(datax)==(A*4)){ # tests if there is data for all years, because some subplots only have data for one or two years 
...
save(Beta1_all_scales, file = "../02_output_data/Beta1_all_scales.RData")
